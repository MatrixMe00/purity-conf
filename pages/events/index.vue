...
<script setup lang="ts">
    ...
    import {events, bigEvent} from "~/composables/events"
    ...
    function comingSoon(){
        return cards.value.filter((card: { otherData: { [x: string]: string } }) => card.otherData["tag"] != 'past' && card.otherData["tag"] != 'live')
    }

    function pastEvent(){
        return cards.value.filter((card: { otherData: { [x: string]: string } }) => card.otherData["tag"] == 'past')
    }

    function liveEvent(){
        return cards.value.filter((card: { otherData: { [x: string]: string } }) => card.otherData["tag"] == 'live')
    }
</script>
